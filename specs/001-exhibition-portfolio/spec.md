...
- Q: Đối với Custom Inertia WebGL Cursor, kiến trúc và cơ chế vận hành như thế nào? → A: Tọa độ chuột được bắn trực tiếp vào Zustand Transient State (GSAP quickSetter, 144Hz). Áp dụng Media Query `(pointer: fine)` để tiêu diệt toàn bộ logic/event chuột trên thiết bị di động (Mobile Extermination). Khi ở chế độ Active, con trỏ có lực hút từ tính (Magnet Effect) vào các khung media. Khi ở chế độ Idle (không di chuột 2.5s), con trỏ chuyển sang trạng thái lơ lửng (breathe/float) tại chỗ để tạo điểm nhấn thị giác.
...
- **Cơ chế WOW**: 
  ...
  - **Zero-DOM-Read Trajectory Math (Khắc phục 165Hz Layout Thrashing)**: Khi ở trạng thái Idle, con trỏ chuột tự động bám theo khối 3D Cube đang di chuyển trên màn hình. Tọa độ $(X, Y)$ của khối 3D Cube MUST được tính toán thuần túy bằng công thức toán hàm Sine dựa trên `scrollY` (hoặc lấy từ RAM store), tuyệt đối **NGHIÊM CẤM gọi `getBoundingClientRect()` hay đọc thuộc tính DOM Layout trong vòng lặp animation rAF/GSAP Ticker** để tránh làm sụt giảm FPS xuống 8 FPS và đẩy CPU lên 30-50%.
...
## 6. BẢN ĐẶC TẢ TỐI ƯU HÓA HIỆU NĂNG TẦNG CSS & MAIN THREAD

**Mục tiêu:** Đạt tối đa 165 FPS, triệt tiêu độ trễ Main Thread và tối ưu hóa VRAM.

### 6.1. TỐI ƯU HÓA CSS OBJECT MODEL (CSSOM)

#### 6.1.1. Cách ly không gian tính toán (Composite Layers & Hardware Acceleration)
...
#### 6.1.2. Kỹ thuật loại bỏ Render (CSS Culling / Content Visibility)
...
#### 6.1.3. Đóng băng tương tác vật lý (Pointer-Events Optimization)
...
#### 6.1.4. Tối ưu luồng dữ liệu truyền tải (CSS Variables Pipeline)
...
### 6.2. TỐI ƯU HÓA BỘ NHỚ VÀ MAIN THREAD (RESOURCES & SCRIPTING)

#### 6.2.1. Giải mã hình ảnh bất đồng bộ (Async Image Decoding)
...
#### 6.2.2. Chiến lược nạp Typography (Font Display Swap)
...
#### 6.2.3. Trì hoãn các tác vụ thứ cấp (requestIdleCallback)
